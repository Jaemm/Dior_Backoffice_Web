...
export default function ProductRecommendationsPage() {
  ...
  const fetchProductGroups = async () => {
    await axios
      .get(
        'https://v2-app.chowis.com/api/dior/product_recommendation_groups',
        {
          headers: {
            'X-CHOWIS-CONSULTANT-TOKEN': token,
          }
    ...
    return(
        <Box className="modal-box" style={{height: '620px', width: '600px'}}>
          ...
          <div className="tab-container" style={{marginTop: '20px'}}>
            <div className={activeTab == 'skincare' ? 'active-tab' : 'tab'} onClick={() => {setActiveTab('skincare')}}>Skincare</div>
            <div className={activeTab == 'makeup' ? 'active-tab' : 'tab'} onClick={() => {setActiveTab('makeup')}}>Make Up</div>
          </div>
          { activeTab == 'skincare' && (
            <>
              <div style={{marginTop: '10px', display: 'flex', alignItems: 'center', flexDirection: 'column'}}>
                <div>Select Recommendation</div>
                <TextField select size='small' variant="outlined" style={{marginTop: '10px', width: '300px'}} onChange={(e) => onChangeRecommendGroup(e.target.value, 'skincare')}>
                  {productGroups.map((e) =>{
                    return(
              ...
              <div style={{marginTop: '10px', background: '#FFFFFF', border: '1px solid #5A5A5A', borderRadius: '10px', padding: '20px'}}>
                ...
                <Grid container spacing={4} alignItems="center">
                  ...
                  <div style={{ width: 400 }}>
                    <ReactSearchAutocomplete
                      items={products.filter(e => e.category === 'UV Protection')}
                      onSearch={handleOnSearch}
                      onHover={handleOnHover}
          ...
          { activeTab == 'makeup' && (
            ...
            <div style={{marginTop: '10px', display: 'flex', alignItems: 'center', flexDirection: 'column'}}>
              <div>Select Recommendation</div>
              <TextField select size='small' variant="outlined" style={{marginTop: '10px', width: '300px'}} onChange={(e) => onChangeRecommendGroup(e.target.value, 'makeup')}>
                  {productGroups.map((e) =>{
                    return(
                      <MenuItem value={e.id}>{e.name}</MenuItem>
                    )
